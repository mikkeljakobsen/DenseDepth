    ...
    for i in range(N//bs):    
        ...
        for j in range(len(true_y)):
            ...
            if save_pred:
                ...
                gt = plasma(gt/10)[:,:,:3]
                pr = plasma(predict(model, x[j]/255, minDepth=settings.MIN_DEPTH*settings.DEPTH_SCALE, maxDepth=settings.MAX_DEPTH*settings.DEPTH_SCALE)[0,:,:,0], vmin=0.0, vmax=5.0)[:,:,:3]
